...
def write_list_id_files(misc_ids, tv_ids, movie_ids, slug, user):
    if len(misc_ids) > 0:
        misc_list_file_path = "special://profile/addon_data/plugin.video.deepthought/misc_from_" + slug + "_by_" + user + ".txt"
    ...
    if len(tv_ids) > 0:
        shows_list_file_path = "special://profile/addon_data/plugin.video.deepthought/shows_from_" + slug + "_by_" + user + ".txt"
        ...
        if not xbmcvfs.exists(shows_list_file_path):
            ...
            batch_add_file.close()
    if len(movie_ids) > 0:
        movies_list_file_path = "special://profile/addon_data/plugin.video.deepthought/movies_from_" + slug + "_by_" + user + ".txt"
...
def write_batch_id_files(tv_ids, movie_ids, misc_ids):
    if len(tv_ids) > 0:
        shows_import_file_path = "special://profile/addon_data/plugin.video.deepthought/shows_to_add.txt"
    ...
    if len(movie_ids) > 0:
        movies_import_file_path = "special://profile/addon_data/plugin.video.deepthought/movies_to_add.txt"
    ...
    if len(misc_ids) > 0:
        misc_import_file_path = "special://profile/addon_data/plugin.video.deepthought/misc_to_add.txt"
...
def lists_trakt_add_liked_to_library():
    ...
    pDialog.create('[COLOR cyan]DeepThought[/COLOR]', 'Creating batch import files')
